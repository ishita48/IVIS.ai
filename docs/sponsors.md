...
| Deepgram | module and token route exist (PR #10) but **nothing in `apps/lens` calls them** — no hook or component imports `lib/deepgram.ts`, so LENS does not qualify yet | `apps/lens/lib/deepgram.ts`, `apps/lens/app/api/deepgram/route.ts` | Fullstack |
